...
public class Serial implements SerialPortEventListener {
	...
	public Serial(Object proxy) {
		this(proxy, "COM1", 9600, 'N', 8, 1);
	}

	
	public Serial(Object proxy, int baudRate) {
		this(proxy, "COM1", baudRate, 'N', 8, 1);
	}

	
	public Serial(Object proxy, String portName) {
		this(proxy, portName, 9600, 'N', 8, 1);
	}

	
	public Serial(Object proxy, String portName, int baudRate) {
		this(proxy, portName, baudRate, 'N', 8, 1);
	}

	
	public Serial(Object proxy, String portName, int baudRate, char parity, int dataBits, float stopBits) {
		this.proxy=proxy;
		// setup parity
		if (parity == 'O') {
	...
	private Method findCallback(final String name) {
		try {
			return proxy.getClass().getMethod(name, this.getClass());
		} catch (Exception e) {
		}
		// Permit callback(Object) as alternative to callback(Serial).
		try {
			return proxy.getClass().getMethod(name, Object.class);
		} catch (Exception e) {
		}
		return null;
	}
	...
	public void pre() {
		if (serialAvailableMethod != null && invokeSerialAvailable) {
			...
			try {
				serialAvailableMethod.invoke(proxy, this);
			} catch (Exception e) {
					...
					if (serialEventMethod != null) {
						if ((0 < bufferUntilSize && bufferUntilSize <= inBuffer-readOffset) ||
							...
							try {
								...
								serialEventMethod.invoke(proxy, this);
							} catch (Exception e) {
